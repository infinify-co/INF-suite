...
class SharingService {
  ...
  async shareDatabase(databaseId, permission = 'view', email = null) {
    try {
      ...
      if (!email && shareToken) {
        const shareLink = `${window.location.origin}/Suite/Oceanbase.html?share=${shareToken}`;
      ...
      listDiv.innerHTML = data.map(share => `
        <div class="share-item">
          <div class="share-info">
            ...
            ${share.share_token ? `
              <div class="share-link">${window.location.origin}/Suite/Oceanbase.html?share=${share.share_token}</div>
            ` : ''}
          </div>
  ...
  async initShareAccess() {
    ...
    if (shareToken) {
      ...
      if (error || !data) {
        ...
        window.location.href = '/Suite/Oceanbase.html';
        return;
      }
